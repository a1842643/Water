...
{
    public class CustomreportDao : _BaseDao
    {
        ...
        internal DataTable getCustomreport(string Branch_No,string SYM,string EYM, string Filecolumn)
        {
            #region 參數告宣
            ...
            _sqlParams.Add("SYM", SYM);
            _sqlParams.Add("EYM", EYM);
            ...
            try
            {
                using (var cn = new SqlConnection(_dbConnPPP))//連接資料庫
                {
                    ...
                    dt.Load(cn.ExecuteReader("getmonth_adv", _sqlParams, commandType: CommandType.StoredProcedure));
                    return dt;
                }
            }
            catch (Exception ex)
