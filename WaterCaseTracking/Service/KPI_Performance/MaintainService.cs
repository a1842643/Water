...
using WaterCaseTracking.Dao.KPI_Performance;
...
namespace WaterCaseTracking.Service.KPI_Performance.Maintain
{
    public class MaintainService
    {
        ...
        #region 抓取分數檔
        public DataTable getKPI_Per(string ddlKPIPerVal)
        {
            #region 參數宣告				
            ...
            #region 流程																
            dt = storedProcedureDao.getKPIPerSimple(ddlKPIPerVal); //將參數送入Dao層,組立SQL字串並連接資料庫
            //拿掉第1、0行不顯示給USER看
        ...
        internal void doLock(string isLocked)
        {
            ...
            if (isLocked == "1")
            {
                ...
                kpi_ScoreDao.DeleteKPI_ScoreFromKPIDis("業績績效");
            }
            if (isLocked == "0")
        ...
        #endregion
        internal int doUpLoad(HttpPostedFileBase file, string NEWID, string funcName, string UserID, string UserName, string ddlKPIPerVal)
        {
            #region 參數宣告		
            ...
            #region 流程	
                manualUploadService.InsertSigningRecord(file, "17", "(業績得分)上傳", NEWID, funcName, UserID, UserName);
            try
            {
                //存Xls轉的DataTable
                DataTable orgDt = new DataTable();
                //抓取分數檔
                dt = storedProcedureDao.getKPIPerSimple(ddlKPIPerVal);
                //判斷檔案室否有trans_date資料
                if (file.ContentLength > 0 || file != null)
                {
                    ...
                    for (int i = 1; i < orgDt.Rows.Count; i++)
                    {
                        for (int j = 2; j < orgDt.Columns.Count; j++)
                        {
                            ...
                            {
                                throw new Exception("第" + (i + 2) + "列資料有誤，請確認" + ex.Message);
                            }
                        }
                    }
        ...
        internal void ImportLastYear()
        {
            ...
            SqlTransaction sqlTrans;
            //刪除今年資料
            kpi_DistributionDao.DeleteKPI_Distribution("業績績效", out sqlConn, out sqlTrans);
            //匯入去年資料
            kpi_DistributionDao.AddKPI_Distribution("業績績效", ref sqlConn, ref sqlTrans);
            #endregion
        }
