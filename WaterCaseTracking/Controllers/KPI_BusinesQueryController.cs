...
using WaterCaseTracking.Models.ViewModels.KPI_BusinesQuery;
...
{
    public class KPI_BusinesQueryController : BaseController
    {
        string FuncName = "經營績效資料查詢";
        // GET: KPI_BusinesQuery
        ...
        public ActionResult Export(ExportViewModel exportViewModel)
        {
            logging(FuncName, "匯出");
            #region 參數宣告
            KPI_BusinesQueryService KPI_BusinesQueryService = new KPI_BusinesQueryService();
            ...
            {
                //查詢修改資料
                dt = KPI_BusinesQueryService.getExportData(exportViewModel);
                if (dt.Rows.Count > 1)
                {
                    fileNamePath = ExportFunction.ExportDataTableTo(dt, exportViewModel.fileExtension, "KPI_BusinesQuery");
                }
            }
            catch (Exception ex)
            {
