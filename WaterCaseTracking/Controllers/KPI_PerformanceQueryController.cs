...
{
    public class KPI_PerformanceQueryController : BaseController
    {
        string FuncName = "業績績效資料查詢";
        // GET: KPI_PerformanceQuery
        ...
        public ActionResult Export(ExportViewModel exportViewModel)
        {
            logging(FuncName, "匯出");
            #region 參數宣告
            KPI_PerformanceQueryService kpi_PerformanceQueryService = new KPI_PerformanceQueryService();
            ...
            {
                //查詢修改資料
                dt = kpi_PerformanceQueryService.getExportData(exportViewModel);
                if (dt.Rows.Count > 1)
                {
                    fileNamePath = ExportFunction.ExportDataTableTo(dt, exportViewModel.fileExtension, "KPI_PerformanceQuery");
                }
            }
            catch (Exception ex)
            {
